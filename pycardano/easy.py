...
class Wallet:
    ...
    def _get_tokens(self):
        ...
        my_policies = {policy.id: policy for policy in get_all_policies(self.keys_dir/"policies") if self.verification_key.hash() in policy.required_signatures}

        token_list = []
        for policy_id, assets in tokens.items():
            for asset, amount in assets.items():
                if policy_id in my_policies.keys():
                    token_list.append(Token(my_policies[policy_id], amount=amount, name=asset))
                else:
